...
elseif RequiredScript == "lib/units/interactions/interactionext" then
	...
	function BaseInteractionExt:interact_start(player, data, ...)
		...
		if HMH:GetOption("stealth_c4") and managers.groupai:state():whisper_mode() and not (VHUDPlus and VHUDPlus:getSetting({"EQUIPMENT", "SHAPED_CHARGE_STEALTH_DISABLED"}, true)) then
