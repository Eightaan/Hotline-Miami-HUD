if not (HMH:GetOption("PLAYER_down") or HMH:GetOption("TEAM_down")) or VHUDPlus and VHUDPlus:getSetting({"CustomHUD", "ENABLED"}, true) and VHUDPlus:getSetting({"CustomHUD", "TEAMMATE", "DOWNCOUNTER"}, true) 
    or WolfHUD and WolfHUD:getSetting({"CustomHUD", "ENABLED"}, true) then return 
end
...
elseif RequiredScript == "lib/managers/hud/hudteammate" then
    ...
    function HUDTeammate:reset_revives()
        if self._revives_counter then
            ...
            if not self._main_player then
                self._revives_counter:set_text(tostring(self._revives_count))
				self._revives_counter:set_color(HMH.Green)
            else
    ...
    function HUDTeammate:set_health(data)
        if data.revives then
            local revive_colors = { HMH.Yellow, HMH.Green, HMH.Green, HMH.Green, HMH.Green}
            self._revives_counter:set_color(revive_colors[data.revives - 1] or HMH.Red)
		...
		if not self._main_player then
		    if Global.game_settings.one_down and self._revives_count == 1 then
		        self._revives_counter:set_color(HMH.Yellow)
		    elseif Global.game_settings.one_down and self._revives_count > 1 then
		        self._revives_counter:set_color(HMH.Red)
