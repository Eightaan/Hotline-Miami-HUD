...
if string.lower(RequiredScript) == "lib/managers/menu/contractboxgui" then
	...
	function ContractBoxGui:init(...)
		...
		if managers.network:session() then
			...
			self:set_loadout_enabled(HMH:GetOption("crew_loadout") and managers.menu:is_pc_controller())
		end
	end
	...
	function ContractBoxGui:set_enabled(...)
		...
		self:set_loadout_enabled(self._enabled and HMH:GetOption("crew_loadout") and managers.menu:is_pc_controller())
	end
...
elseif string.lower(RequiredScript) == "lib/managers/menu/crimespreedetailsmenucomponent" then
	...
	function CrimeSpreeDetailsMenuComponent:populate_tabs_data(tabs_data, ...)
		...
		if not self:_is_in_preplanning() and not Global.game_settings.single_player and HMH:GetOption("crew_loadout") then
	...
	for id, ptr in pairs(TeamLoadoutItem) do
		if type(ptr) == "function" then
			TeamLoadoutCustom[id] = TeamLoadoutCustom[id] or function(self, ...)
				log( "TeamLoadoutCustom Unoverwritten function call WARNING!" )
			end
		end
	end
	if CoreClass and HMH:GetOption("crew_loadout_briefing") then
